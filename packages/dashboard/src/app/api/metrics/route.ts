...
export async function GET(_req: NextRequest) {
  ...
  const [monthTotal, cancelledMonth, uniqueClients, revenueResult] =
    await Promise.all([
      ...
      //Cuantos clientes distintos tuvieron al menos un turno confirmado este mes
      Appointment.count({
        where: {
          tenant_id: tenantId,
          status: "confirmed",
      ...
      Appointment.findOne({
        ...
        attributes: [[fn("SUM", col("service.price")), "total"]],
        include: [{model: Service, as :"service", attributes: []}],
        raw: true
      }),
    ]);

    const revenue = +(revenueResult as any)?.total || 0
